...
nn4_small2.load_weights('weights/nn4.small2.v1.h5', by_name=True)
...
for path in test_paths:
    ...
    for i in range(test_embs.shape[0]):
        ...
        results.append({'people': np.argsort(distances)[:1], 'min_distance': np.min(distances)})

    names = []

    for result in results:
        if result['min_distance'] > threshold:
            name = "unknown"
        else:
            name = df_train[(df_train['label']==result['people'][0])].name.iloc[0]
        names.append(name)
    ...
    for i,faceRect in enumerate(faceRects):
        ...
        cv2.putText(show_image,'{} ({})'.format(names[i], results[i]['min_distance']),(x1,y1-5), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,255),1,cv2.LINE_AA)
